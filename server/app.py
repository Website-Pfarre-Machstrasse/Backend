...
def create_app():
    ...
    cors = CORS(resources=app.config.get('CORS'))

    import tinify
    tinify.key = app.config.get('TINIFY_KEY')
    # endregion configure app
    ...
    from .resources import UserResource, LoginResource, RefreshResource

    api.add_resource(UserResource, '/user', endpoint='user')
    api.add_resource(LoginResource, '/login', endpoint='login')
    api.add_resource(RefreshResource, '/refresh', endpoint='refresh')
...
def setup_schema():
    ...
    for class_ in db.Model._decl_class_registry.values():
        if hasattr(class_, "__tablename__") and not hasattr(class_, '__no_marshmallow__'):
            ...
            setattr(class_, "__marshmallow__", lazy_property(schema_class))
